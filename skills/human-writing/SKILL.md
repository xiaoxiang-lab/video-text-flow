...
视频文案的**立场必须是用户的立场，不是素材自带的**（素材=文案/标题/话题/数据，立场来自用户或商量结果）。

### 立场重写（素材立场 ≠ 用户立场时）

保留：事实、数据、时间线、话题方向、标题候选。
替换：立场表达——结论、判断、论证方向、设问角度（按冲突理论：立场鲜明、有冲突、可反驳）。
重写 = 重新论证（证据不变，结论和论证方向换成用户的），不是润色。

硬约束：
- 事实/数据逐项保留，子 agent 复核「事实未变 + 立场已换 + 无杜撰」
- **成稿禁令同样适用（硬规则，实测教训）**：立场重写稿也是正文，必须通过「成稿绝对不能出现」清单（提示性冒号、同构排比 ≤2 项、破折号、翻案腔、黑话、长列举单口气超 20 秒拆开）——重写时只记立场规则忘了成稿禁令 = 丢规则，实测 v1/v2 漏了 8 处冒号
- 产出交关卡1（立场重写稿确认）用户确认后才算定稿。**用户确认立场 ≠ 规则合规**：产出物必须有「规则复核」状态（子 agent 按固定清单查），用户确认与规则复核分开记录

### 按立场从零写（路径2：用户直接给立场/标题）

先定主张/立场再动笔（冲突理论：立场鲜明才有评论，放之四海皆准无人留言），正文立场与选定标题一致，产出同样交关卡1 确认。

## 视频分镜稿模式（口播稿 → 分镜稿）

用户要求把口播稿/正文做成「能拆镜的视频配音文本」时启用本模式。一句话定位：**信息保留、结构重构、口语保持**——它是对口播稿的「视频化改编」，不是重写新文章，也不是普通润色。

### 可拆性自查（5 条标准，进 VOX 前必查）

| # | 标准 | 说明 |
|---|---|---|
| 1 | 单句朗读 ≤9 秒（约 ≤60 字） | 长句按逗号/句号/停顿点拆开 |
| 2 | 一句一知识点 | 每句只讲一件事（数据/观点/问题/证据），不夹带 |
| 3 | 相邻 3 镜不连续同类型 | 连续证据/数字连发 ≤2 句，超过就穿插评点、转折或提问。**红线只针对修辞型（设问/立场/过渡/评点）**；定义/论证链（如「Harness 是什么」的连续定义段）是逐字切分下无解的正常结构，不适用本红线 |
| 4 | 无纯过渡句 | 每句都有信息承载；「看这一串动作」「由此可见」类删或并入前后句 |
| 5 | 每句有视觉落点 | 能对应一个主体/动作/场景，纯抽象论述需转成可拍意象 |

**拆分规则（硬约束，防残句）**：按「可独立朗读」切，拆出的分句不得 <12 字（衔接词「而是/甚至/那这」起头的超短残句必须合回）；拆完每镜单独朗读一遍，读起来像话没说完 = 拆错，合回。拆后合回后的镜 ≤9s 就合法（不强制拆到极限）。

5 条全部满足 = 达标，口播稿可直接进 VOX 拆镜。任一违反 = 不达标，必须改编成分镜稿。

### 改编三操作（每句必须标注来源，防漂移）

- **拆长句**：标注「拆自原文第 X 句」
- **删过渡**：标注「删掉原文第 X 句」
- **新增评点句**：标注「【新增】」，用于打破连续数据段、穿插观点/提问

无来源标注的句子 = 擅自重写，禁止。改编时**不润色措辞、不丢信息主张、不写书面语**——分镜稿必须保持可念（它是配音文本）。

### 输出

分镜稿.md：每句标注来源的操作清单 + 5 条标准自查结论（哪些句违反哪条、如何处理）。用户确认后才算定稿。

### 复核留痕（强制，交付前必做）

分镜稿必须经 `task` 子 agent 独立复核（禁止主 agent 自查代替），文件内必须附「复核记录」块，无此块 = 未完成交付。**复核清单固定写死，子 agent 按此清单逐项查（不由主 agent 临场转述——转述会丢维度，实测教训）**：
...
检查项（逐项 ✓/✗）：
  - 单句朗读 ≤9s（超时 0 容忍）
  - 一句一知识点
  - 相邻3镜不连续同类型（红线只针对修辞型：设问/立场/过渡/评点；定义/论证链豁免）
  - 无纯过渡句
  - 每句有视觉落点
  - 每句有来源标注（无来源句 0 容忍）
  - 镜数：实际镜数与自称一致（逐镜数）
  - 残句：无 <12 字残句；无「如果/因为」等条件从句独立成镜；每镜能独立朗读
  - 标注真实性：来源标注逐条对原文核验
异常：N 项（列表）
修正：N 项（列表）
```

规则版本：记录本次复核依据的规则状态（规则更新后旧产物必须按新规则回扫重查）。

阈值：超时句/无来源句 0 容忍，直接改；语义异常（信息丢失/新增无来源）≥2 处 → 重写该段后重交；全过则写「0 异常」。
